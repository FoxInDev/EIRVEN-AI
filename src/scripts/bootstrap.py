...
class Bootstrap:
    ...
    def install_once(self) -> None:
        try:
            ...
            marker = ROOT / ".installed-v2.4.0-r72-em"
    ...
    def install(self) -> None:
        ...
        self.done_units = 0.0
        try:
            state_file.write_text(json.dumps({"version": "2.4.0", "build": "r72-em", "status": "running", "updated": time.time()}), encoding="utf-8")
            self.install_once()
            state_file.unlink(missing_ok=True)
        except Exception as exc:
            state_file.write_text(json.dumps({"version": "2.4.0", "build": "r72-em", "status": "failed", "error": str(exc)[-2000:], "updated": time.time()}, ensure_ascii=False), encoding="utf-8")
