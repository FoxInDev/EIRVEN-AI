...
if _install_override:
    ...
    APP_ROOT = ROOT
DEFAULT_PORT = 7860
CURRENT_BUILD = "r72-em"
INSTALL_MARKER = ".installed-v2.4.0-r72-em"
    ...
    if installed_exe.resolve() != running_exe:
        ...
        try:
            ...
            try:
                ...
                pass
    try:
        (APP_ROOT / ".payload-r72-em").write_text("2.4.0 r72-em\n", encoding="ascii")
    except OSError:
        pass
...
def _configured_model() -> str:
    try:
        ...
        return str(detect_hardware().recommended_main_model or RELEASE_TEXT_MODEL)
    except Exception:
...
def _configured_vision_model() -> str:
    try:
        ...
        if str(source) not in sys.path:
            sys.path.insert(0, str(source))
        from eirven_ai.hardware import detect_hardware
        return str(detect_hardware().recommended_vision_model or RELEASE_VISION_MODEL)
    except Exception:
