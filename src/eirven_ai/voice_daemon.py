...
class NativeVoiceDaemon:
                ...
                while not self._stop.is_set():
                    ...
                    if not recording:
                        ...
                        if speaking_now:
                            ...
                            likely = self._start_frame_likely(rms, vad_speech, self._noise_floor)
    ...
    def _looks_like_recent_echo(self, text: str) -> bool:
        ...
        if not heard or not spoken or time.monotonic() - self._last_spoken_at > 8.0:
            return False
        if len(heard) >= 10 and (heard in spoken or spoken in heard):
            return True
        return len(heard) >= 8 and SequenceMatcher(None, heard, spoken).ratio() >= 0.84
    ...
    def _wake_variants(self) -> set[str]:
        # Voice activation follows the assistant name selected in settings.  ASR tolerance
        # is handled below by phonetic/fuzzy matching against this configured name; no old
        # product nickname remains active after the owner renames the assistant.
        wake=self._normalize(self._wake_phrase())
        return {wake} if wake else {"эйрвен"}

    def _wake_phrase(self) -> str:
        try:
            return str(self.services.identity.get().assistant_name or "Эйрвен").strip() or "Эйрвен"
        except Exception:
            return "Эйрвен"
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                if not text or text.casefold().startswith("речь не распознана"):
                    ...
                    continue
                if self._looks_like_recent_echo(text):
                    try:
                        runtime = getattr(self.services, "runtime", None)
                        if runtime is not None: runtime.voice_activity_resolved(accepted=False)
                    except Exception: pass
