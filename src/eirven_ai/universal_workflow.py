...
class UniversalWorkflowEngine:
    ...
    def _current_window_text_fastpath(self, goal: str, *, text: str = "") -> dict[str, Any] | None:
        ...
        if not submitted:
            return {"ok":False,"completed":False,"verified":False,"method":"grounded-input","error":"Текст подтверждён в поле, но Enter не отправился"}
        self._trace("UNIVERSAL_FOCUSED_TEXT",goal=goal,title=acquired.get("title"),chars=len(payload),verified=True,completed=True,focus_verified=acquired.get("focused"))
        return {"ok":True,"completed":True,"submitted":True,"verified":True,"method":"grounded-input","title":acquired.get("title"),"text_chars":len(payload),"evidence":"field_value_confirmed_before_submit"}
