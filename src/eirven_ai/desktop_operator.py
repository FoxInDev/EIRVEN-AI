...
class DesktopOperator:
    ...
    def type_verified(self, acquired: dict[str, Any], text: str, *, submit: bool = False, require_verified: bool = True) -> dict[str, Any]:
        ...
        time.sleep(.18)
        rows=self._elements(title,limit=360,handle=handle); payload=self._norm(text)
        field_evidence=False; visible_evidence=False; focused=False
        for e in rows:
        ...
        verified=bool(field_evidence or (focused and visible_evidence))
        ...
        self._trace("OPERATOR_TYPE_VERIFY",title=title,chars=len(text),verified=verified,focused=focused,submit=submit,completed=completed,clipboard_roundtrip=clipboard_roundtrip,clipboard_used=clipboard_used,purpose=str(acquired.get("purpose") or ""))
        ...
        return {"ok":True,"completed":completed,"verified":verified,"typed":True,"submitted":bool(submit and completed),"title":title,"evidence":"uia_value" if field_evidence else ("clipboard_roundtrip" if clipboard_roundtrip else "focused_visible")}
    ...
    def telegram_send(self, recipient: str, text: str) -> dict[str, Any]:
        ...
        if not typed.get("ok"): raise RuntimeError("Поле поиска Telegram найдено, но имя в нём не подтвердилось")
        time.sleep(.18)
        # Wait for actual clickable result rows, not duplicate Text labels/chips.
        ...
        if not self.tools.execute("press_key",{"key":"enter"}).get("ok",False):
            raise RuntimeError("Не удалось отправить Enter в подтверждённое поле сообщения")
        # Verify only after send; never press Enter/click Send a second time for the same payload.
        end=time.monotonic()+5.0; verified=False; composer_empty=False; text_visible=False
        while time.monotonic()<end:
            ...
            for e in current:
                ...
                if any(x in blob for x in ("input message","input-message","write a message","сообщение")) and self._norm(text) not in blob:
                    composer_empty=True
            if text_visible or (composer_empty and self._ui_fingerprint(current)!=before_sig):
                verified=True; break
            time.sleep(.18)
        self._trace("OPERATOR_VERIFY",app="telegram",action="send",recipient=recipient,verified=verified,text_visible=text_visible,composer_empty=composer_empty,active_chat=True)
        if not verified:
            return {"sent":True,"verified":False,"completed":True,"recipient":recipient,"client":client,"error":"Сообщение отправлено один раз, но появление bubble не подтверждено"}
        return {"sent":True,"verified":True,"completed":True,"recipient":recipient,"client":client}
    ...
    def yandex_wave(self) -> dict[str, Any]:
        ...
        rows_now=live_rows()
        play=next((e for e in rows_now if e.get("visible",True) and e.get("enabled",True) and self._norm(e.get("control_type"))=="button" and "vibeplayercontrols playbutton" in self._norm(e.get("class_name")) and len(e.get("rectangle") or [])==4 and int((e.get("rectangle") or [0,0,0,0])[1])>500),None)
        if not play:
            play=find_live(["Воспроизведение","Воспроизвести","VibePlayerControls_playButton","play"],types=("Button",))
        if not play:
