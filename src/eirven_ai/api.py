...
def build_api(services: Services) -> FastAPI:
    ...
    def system_shutdown() -> dict[str, Any]:
        """Gracefully stop EIRVEN even when it was launched without the supervisor."""
        ...
        def request_stop() -> None:
            try:
                ...
                pass
            # Give the HTTP response time to flush, then ask the desktop companion to hide.
            time.sleep(0.75)
            try:
                services.companion.hide()
            except Exception:
                pass
            # If the app was not started under the supervisor, or the stop marker is still
            # present, finish the process explicitly so the UI never hangs on "Отключаю…".
            time.sleep(1.35)
            try:
                if stop_file.exists():
                    os._exit(0)
        ...
        return {"ok": True, "scheduled": True, "pid": server_pid}
