...
>>>>>>> b48a166 (fix: repair mini orb, autostart and Telegram sending)
    ...
    def _telegram_send_turn(self, target: str) -> tuple[bool,str,dict[str,Any]]:
        ...
        if not platform:
            self.db.set_setting("pending_send", {"recipient":recipient,"platform":"","message":message,"awaiting":"platform","at":time.time()})
            return True,"В каком мессенджере отправить сообщение? Например, в Telegram.",{"action":"send_need_platform","model":"deterministic"}
        if not recipient:
            self.db.set_setting("pending_send", {"recipient":"","platform":platform,"message":message,"awaiting":"recipient","at":time.time()})
            return True,"Кому именно отправить сообщение?",{"action":"send_need_recipient","model":"deterministic"}
        if not message:
            self.db.set_setting("pending_send", {"recipient":recipient,"platform":platform,"message":"","awaiting":"message","at":time.time()})
    ...
    def _pending_send_turn(self, query: str) -> tuple[bool, str, dict[str, Any]]:
        ...
        if not isinstance(pending, dict) or not pending:
            return False, "", {}
        try:
            age = time.time() - float(pending.get("at") or 0)
        ...
        if confirming:
            if is_affirmative_confirmation(query):
                self.db.set_setting("pending_send", {})
                return self._telegram_send_turn(f"{recipient} в {platform or 'telegram'} сообщение {message}")
            # Social replies are never interpreted as a send confirmation. Keep the
            # draft available, but let the normal instant-chat route answer immediately.
            if self._instant_reply(query) is not None:
                return False, "", {}
            if detect_command(query) is not None or len(clean.split()) > 6:
                self.db.set_setting("pending_send", {})
                return False, "", {}
            return True, f"Отправить {recipient or 'получателю'} текст «{message}»? Скажи «да, отправляй» или «отмена».", {"action":"send_waiting_confirmation","model":"deterministic","needs_user":True}

        # A greeting is a conversation, even if an old send dialogue is still pending.
        # This rule is deliberately evaluated before raw-field continuation.
        if self._instant_reply(query) is not None and not is_resume_confirmation(query):
            return False, "", {}
        ...
        if recipient and platform and message:
            ...
            self.db.set_setting("pending_send", state)
            return True, f"Отправить {recipient} текст «{message}» в Telegram? Скажи «да, отправляй» или «отмена».", {"action":"send_confirmation","model":"deterministic","needs_user":True}

        if changed:
            awaiting = "platform" if not platform else ("recipient" if not recipient else "message")
            self.db.set_setting("pending_send", {"recipient":recipient,"platform":platform,"message":message,"awaiting":awaiting,"at":time.time()})
    ...
    def _self_shutdown_requested(query: str) -> bool:
        ...
        return bool(re.fullmatch(
            r"(?:пожалуйста\s+)?(?:(?:эрви|эйрвен|eirven)[, :!-]*)?(?:выключи|(?:(?:выключи|закрой|заверши|останови)\s+(?:себя|эрви|эйрвен|eirven)(?:\s+полностью)?)|"
    ...
    def _hosting_from_text(text: str) -> str:
        ...
        known = re.search(
            r"\b(vercel|netlify|cloudflare|github\s+pages|gitlab\s+pages|beget|reg\.ru|timeweb|"
    ...
    def _phone_guidance_turn(self, query: str, conversation_id: str) -> tuple[bool, str, dict[str, Any]]:
        ...
        try:
            ...
            return True, f"Вход подтверждён, но окно привязки не запустилось: {exc}", route
    ...
    def _priority_control_turn(self, query: str, conversation_id: str) -> tuple[bool, str, dict[str, Any]]:
        ...
        control_only = bool(
            ...
            or is_pc_shutdown_request(query)
            or is_pc_shutdown_cancel_request(query)
            or is_phone_setup_request(query)
            or is_chat_pairing_request(query)
            or bool(self.db.get_setting(self._power_confirmation_key(conversation_id), {}))
        )
        if control_only:
    ...
    def _instant_reply(self, query: str) -> str | None:
        ...
        normalized = re.sub(r"^(?:эрви|эйрвен|eirven)\s*[,;:!\-]*\s*", "", normalized, flags=re.I)
        # A noisy 35-second r28 capture often returned the repeated sequence
        # ``Привет. Эрве. Эрви, привет``.  It is still an unambiguous greeting and
        # must never cold-start the LLM.  Collapse only when every non-greeting token is
        # a close wake-name variant; ordinary phrases remain untouched.
        tokens = re.findall(r"[a-zа-яё0-9_-]+", normalized, re.I)
        greeting_tokens = {"привет", "здравствуй", "здравствуйте"}
        wake_tokens = {"эрви", "эйрвен", "eirven"}
        if tokens and any(token in greeting_tokens for token in tokens):
            def _wake_like(token: str) -> bool:
                return token in wake_tokens or max(
                    (SequenceMatcher(None, token, wake).ratio() for wake in wake_tokens),
                    default=0.0,
                ) >= 0.67

            if all(token in greeting_tokens or _wake_like(token) for token in tokens):
                normalized = next(token for token in tokens if token in greeting_tokens)
        ...
        *,
        ...
        if is_resume_confirmation(query):
            with self._lock:
                ...
                already_resuming = bool(already_resuming and not already_resuming.is_set())
            if already_resuming:
        ...
        if brief_acted:
            ...
            if self.runtime is not None:
                try: self.runtime.finish(brief_answer, ok=True)
                except Exception: pass
            return
        ...
        if not image_paths and not (autonomous_pending or workflow_pending):
