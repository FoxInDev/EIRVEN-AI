...
async def lifespan(app):
    ...
    # Cold-start governor: ASR and the fixed Baya voice load in parallel. Listening is
    # exposed immediately after the ASR inference probe; the action model waits for both
    # voice workers so it cannot steal CPU/RAM during their cold load.
    ...
    try:
        log_event(services.settings.root_dir, "RESOURCE_POLICY", action_model_prewarm=True, tts_preload=True, after="stt_and_baya_parallel", keep_alive=services.settings.keep_alive)
    except Exception:
        pass
