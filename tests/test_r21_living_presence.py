...
def test_configured_wake_is_one_shot_five_seconds(tmp_path: Path):
    daemon = _daemon(tmp_path)
    assert daemon._wake_variants() == {"эйрвен"}
    ...
    armed = daemon._activation_route("эйрвен", speech_started_at=11.0, now=11.2)
    assert armed["action"] == "arm"

    direct = daemon._activation_route("эйрвен открой телеграм", speech_started_at=12.0, now=12.2)
...
def test_ui_is_living_glass_and_settings_are_reduced():
    ...
    assert "wake_phrase" in js and "wakeName" in js
